 ...
 */
public class ZmOidcProfileCreator<P extends OidcProfile> extends OidcProfileCreator<P> {
    private static final Logger logger = LoggerFactory.getLogger(ZmOidcProfileCreator.class);

    public ZmOidcProfileCreator(final OidcConfiguration configuration, final OidcClient client) {
    ...
    public Optional<UserProfile> create(final OidcCredentials credentials, final WebContext context) {
        ...
        try {
            ...
            // keep the session ID
            final String sid = Optional.ofNullable((String) claimsSet.getClaim(Pac4jConstants.OIDC_CLAIM_SESSIONID)).orElseGet(() ->{
                return (String) claimsSet.getClaim(JwtClaims.SUBJECT);
            });
            if (isNotBlank(sid)) {
